import React from 'react';

function CloudSyncIcon({ size = 14, color = "currentColor", direction = 'up' }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden>
      {/* Cloud outer shell with a gap at the bottom for the arrow */}
      <path d="M4 14.899A7 7 0 1 1 15.71 8h1.79a4.5 4.5 0 0 1 2.5 8.242" />
      {/* Arrow */}
      <g className={`cloud-arrow cloud-arrow-${direction}`}>
        <path d="M12 12v9" />
        {direction === 'up' ? (
          <path d="m16 16-4-4-4 4" />
        ) : (
          <path d="m8 17 4 4 4-4" />
        )}
      </g>
    </svg>
  );
}

export default CloudSyncIcon;
